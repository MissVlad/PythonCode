...
# train&validation - 2020.3.23 to 2021.4.30; test - 2021.5.1 to 2021.10.24
# 数据的train、validation、test集划分，以及标准化在MATLAB完成
data = loadmat(project_path / 'data/processed_data/nn_model_data/XY_data_Model2.mat')
Xtrain = data['xtrain']
Xvali = data['xvali']
Xtest = data['xtest']
Ytrain = data['ytrain']
...
XTrain = np.reshape(Xtrain, (240, 24, 13))
YTrain = np.reshape(Ytrain, (240, 24, 1))
XVali = np.reshape(Xvali, (60, 24, 13))
YVali = np.reshape(Yvali, (60, 24, 1))
XTest = np.reshape(Xtest, (170, 24, 13))
YTest = np.reshape(Ytest, (170, 24, 1))
# %% model structure
model = Sequential()
model.add(Bidirectional(LSTM(1024, input_shape=(24, 14), return_sequences=True)))
...
model = tf.keras.models.load_model('elePriceForecast_net_afterCovid19_Model2.h5')
test_prediction = model.predict(XTest)
y_prediction = test_prediction * 388.8 - 38.8
...
pretest_withTime.to_excel('preElePrice_testModel2.xlsx')
acttest_withTime.to_excel('actElePrice_testModel2.xlsx')
...
from Ploting.uncertainty_plot_Func import *
